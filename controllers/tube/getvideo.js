...
const { resolveToken } = require("../../db/navTokens.js");

// POST化された動画遷移用: トークンから実際の動画IDを解決し、
// 従来の /:id ルートへ内部的に委譲する。
// URL上にvideoIdが直接出ないようにするための仕組み。
router.get("/t/:token", async (req, res, next) => {
  const payload = await resolveToken("watch", req.params.token).catch(() => null);
  ...
  // list/server などの付帯情報をクエリに復元してから /:id ハンドラへ委譲する
  const qs = new URLSearchParams(req.query);
  if (payload.list) qs.set("list", payload.list);
  if (payload.server) qs.set("server", payload.server);
  const qsStr = qs.toString();
  req.url = "/" + encodeURIComponent(payload.videoId) + (qsStr ? "?" + qsStr : "");
  next();
});
